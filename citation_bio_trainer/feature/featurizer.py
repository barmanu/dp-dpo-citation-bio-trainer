...
class Featurizer(object):
    def __init__(self, feat_config):
        self.doc_sequencer = LSTMFeaturizer(
    ...
    def fit_transform(self, textlist, taglist):
        ...
        if self.feat_config["spacy_feats"]:
            ...
            data_dict["spacy_num_feats"] = np.array(
                [i.tolist() for i in spacy_feats_padded]
            )
    ...
    def transform(self, textlist, taglist=[], has_tags=True):
        ...
        if len(taglist) == 0:
            has_tags = False
        padded_textarray, padded_tagarray = pad_sequences(
            textlist, self.maxlen, taglist, has_tags
        )
        if has_tags:
            encodedLabel = np.array(
                [[self.tags2index[w] for w in s] for s in padded_tagarray]
        ...
        if self.feat_config["spacy_feats"]:
            ...
            for ind in range(len(spacy_feats)):
                ...
                if len(spacy_feats[ind]) <= self.maxlen:
                    spacy_mask[0 : len(spacy_feats[ind]), :] = spacy_feats[
                        ind
                    ][:]
                else:
