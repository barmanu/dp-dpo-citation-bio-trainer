...
class SpacyFeaturizer(object):
    ...
    def get_array_from_df_combined(self, df):
        ...
        nlp.tokenizer = Tokenizer(nlp.vocab) ## to make spacy not to split in ' and other characters
        ...
        if self.cat_features:
            spacy_bigint_attributes = self.spacy_vorn_attributes + self.spacy_vocab_attributes
        ...
        if self.num_features:
            tokens_features_smallint = doc.to_array(self.spacy_num_attributes).astype('int8')
    ...
    def get_spacy_dask(self, df, blocksize=100):
        ...
        .map_partitions(self.get_array_from_df_combined, meta = [('spacy_bin', 'object'), ('spacy_cat', 'object')])\
