...
class WindowDNN:
    ...
    def get_model(self):
        ...
        x = tf.keras.layers.concatenate(inputs=embedding_out, axis=-1, name="cat")
        x = tf.keras.layers.Permute((2, 1))(x)
        x = tf.keras.layers.MaxPooling1D(pool_size=2, strides=None, padding="valid", data_format="channels_last")(x)
        x = tf.keras.layers.Permute((2, 1))(x)
        if self.config.get("drop_rate", 0.0) > 0.0:
            x = tf.keras.layers.Dropout(self.config.get("drop_rate", 0.0))(x)
        x_out = tf.keras.layers.Dense(self.config.get("hidden_dim", 10000), activation="relu", name="hidden")(x)
