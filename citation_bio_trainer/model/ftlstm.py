...
def get_model(feat_config, model_config, maxlen):
    try:
        ...
        print("after imput")
        y = Embedding(
            input_dim=feat_config["max_vocab"],
            input_length=maxlen,
            output_dim=300,
            weights=[model_config["embedding"]["matrix"]],
            trainable=model_config["embedding"]["trainable"],
        )(input)
        print("after embedding")
        if (
            model_config["aux_feats"]["use"]
            and model_config["aux_feats"]["place"] == "before_lstm"
        ):
        ...
        if (
            model_config["aux_feats"]["use"]
            and model_config["aux_feats"]["place"] == "after_lstm"
        ):
            aux_feats = Input(shape=(maxlen, model_config["aux_feats"]["dim"]))
            y = Concatenate()([y, aux_feats])
        ...
        if model_config["dense"]["use"]:
            y = Dense(
                model_config["dense"]["units"],
                activation=model_config["dense"]["activation"],
            )(y)

        if model_config["timedistributed"]["use"]:
            y = TimeDistributed(
                Dense(1, activation=model_config["output_activation"])
            )(y)
        else:
