...
class BIOLSTM:
    ...
    def get_model(self):
        ...
        if self.l2 == 0.0:
            lstm = Bidirectional(
                LSTM(
                    units=self.hidden_dim, return_sequences=True, name="lstm-0"
                ),
                merge_mode="concat",
                name="b_cat_rnn",
            )
        ...
        drop = Dropout(self.drop_rate, name="drop")
    ...
    def np_array_2_cv_split(x, y, split=5):
        ...
        for i in range(split - 1):
            x_splits.append(
                x[start : min((start + sample_per_split), len(x)), :]
            )
            y_splits.append(
                y[start : min((start + sample_per_split), len(y)), :]
            )
    ...
    def train_model(
        ...
        train_history.rename(
            columns={train_history.columns[0]: "epoch"}, inplace=True
        )
        metrics = (
            history.history["val_loss"],
            history.history["val_binary_accuracy"],
            history.history["val_ser"],
